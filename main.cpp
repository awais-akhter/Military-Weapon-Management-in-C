...
int main() {
	PlaySound("beam.wav", NULL, SND_ASYNC);
	PlaySound("laser.wav", NULL, SND_ASYNC);
	first();
	PlaySound("mgun.wav", NULL, SND_ASYNC);
	system("pause");
	...
	bool verify = login();
	PlaySound("trooper.wav", NULL, SND_ASYNC);
	...
	PlaySound("9mm.wav", NULL, SND_ASYNC);
	scanf("%d", &choice);
	if (choice == 1) {
		createDatabase();
	}
...
void delEnd() {
	...
	if (node == NULL) { //check if list is already empty
		red();
		printf("\n\t\t\t\t************ List is empty already! ************\n\n");
		PlaySound("trooper.wav", NULL, SND_ASYNC);
		return;
